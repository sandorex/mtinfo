                    ...
                    size_t                                     length)
{
...

    {
        ...
        for (auto&& offset : offsets) {
            ...
            const auto null_ch_position
              = strings_table_raw.find ('\0', offset.value());
            if (null_ch_position == std::string::npos)
                throw mtinfo::ErrorParsing (
                  "cannot find the end of a string in the strings table, the "
                  "file is probably corrupt");

            string_table[count++]
              = strings_table_raw.substr (offset.value(), null_ch_position);
        }
    }
...
// parses curses extended capabilities header
template <class BeginIt, class EndIt>
std::array<int16_t, 5>
parse_ext_header (BeginIt& begin, EndIt& end)
{
    if (begin + 5 >= end)
        throw mtinfo::ErrorEOF ("the extended info header");
...
parse_terminfo (BeginIt begin, EndIt end)
{
...

    {
        ...
        const int16_t magic_number = header[0];
        if (magic_number != MAGIC_NUMBER)
            throw mtinfo::ErrorParsing ("invalid magic number");

        const int16_t names_section_size_bytes        = header[1];
        const int16_t bool_section_size_bytes         = header[2];
        const int16_t num_section_size_shorts         = header[3];
        const int16_t str_offsets_section_size_shorts = header[4];
        const int16_t str_table_section_size_bytes    = header[5];
        ...
        // parse only if there are any strings
        if (strings_offsets.size() > 0) {
            // ensure no undefined bools are written
