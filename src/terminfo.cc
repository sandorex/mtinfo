            ...
            if (numbers_raw.size() > 0)
                for (size_t i = 0; i < numbers_raw.size(); ++i) {
                    if (numbers_raw[i] == -1)
                        ...
                        terminfo.bools[i] = numbers_raw[i];
                }
        }
